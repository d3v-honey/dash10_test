...
class Exporter {
    ...
    function getTotals($attempt, $shot) {
        return $attempt ? (round($shot / $attempt, 2) * 100) . '%' : 0;
    }
    ...
    function getPlayerStats($search) {
        ...
        foreach ($data as &$row) {
            ...
            $row['field_goals_pct'] = $this->getTotals($row['field_goals_attempted'], $row['field_goals']);
            $row['3pt_pct'] = $this->getTotals($row['3pt_attempted'], $row['3pt']);
            $row['2pt_pct'] = $this->getTotals($row['2pt_attempted'], $row['2pt']);
            $row['free_throws_pct'] = $this->getTotals($row['free_throws_attempted'], $row['free_throws']);
